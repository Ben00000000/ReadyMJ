...
for (let i = 1; i <= 7; i++) {
    walkRightImageUrls.push(`https://raw.githubusercontent.com/Ben00000000/MJ-v3/main/right%20(${i}).png`);
}

const walkLeftImageUrls = [];
for (let i = 1; i <= 7; i++) {
    walkLeftImageUrls.push(`https://raw.githubusercontent.com/Ben00000000/MJ-v3/main/right%20(${i}).png`);
}
...
function handleMove(direction) {
    ...
    if (direction === 'left') {
        ...
        currentPosition = Math.min(window.innerWidth - character.clientWidth, currentPosition + step);
        character.style.left = `${currentPosition}px`;
